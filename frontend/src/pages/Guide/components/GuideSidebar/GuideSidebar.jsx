...
export default function GuideSidebar() {
  ...
  return (
    <aside className={styles.sidebar}>
      <button type="button" className={styles.brand} onClick={() => navigate("/guide/dashboard")} aria-label="Open guide dashboard">
      ...
      <nav className={styles.navigation} aria-label="Guide navigation">
        {navigationItems.map((item) => {
          ...
          return (
            <NavLink key={item.to} to={item.to} end={item.end} className={({ isActive }) => `${styles.navItem} ${isActive ? styles.active : ""}`}>
      ...
      <a className={styles.supportCard} href="mailto:support@nefru.com">
        <span className={styles.supportIcon}><CircleHelp size={21} aria-hidden="true" /></span>
        <span><strong>Need Help?</strong><small>Contact Support</small></span>
