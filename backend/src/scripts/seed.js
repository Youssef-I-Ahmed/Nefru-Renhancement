...
// Legacy demo seed prices were authored as small USD-like values. Seed new data in EGP.
const SEED_LEGACY_USD_TO_EGP = 50;
...
async function createTrips(guideUsers) {
  ...
  return Trip.create(
    tripSeedData.map((trip) => ({
      ...
      price: Math.round(trip.price * SEED_LEGACY_USD_TO_EGP),
      currency: "EGP",
  ...
  for (let index = 0; index < bookings.length; index += 1) {
    ...
    if (booking.paymentStatus === "paid") {
      notifications.push({
        ...
        message: `Your payment of EGP ${booking.totalPrice.toLocaleString("en-US")} for ${trip.title} was completed.`,
        isRead: index % 2 === 0,
